 ...
 * Date: 03/10/2018
 * Time: 15:21
 * Description: Get stories of SP Oktoberfest influencers on instagram and save all on database (this job belongs to louder 1.0)
 */
...
class GetStoriesNissin extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'Instagram:V1.GetStoriesNissin';
    ...
    public function __construct(Client $guzzle, Carbon $carbon)
    {
        $this->_guzzle	= $guzzle;
        $this->_carbon	= $carbon;
        $this->pathS3   = "1/nissin/stories/{$this->_carbon->format('Y')}/";
        parent::__construct();
    }
    ...
    public function handle()
    {
        try {
            ...
            foreach ($programs as $program) {
                $influencers = DB::connection('nissin')
                                 ->select('SELECT 
                                            * 
                                           FROM 
                                            Influencers 
                ...
                foreach ($influencers as $influencer) {
                    ...
                    if (!$responseStories['user']['is_private']) {
                        ...
                        if (strrpos($influencer->nome, $responseStories['user']['full_name']) === false) {
                            ...
                            $resultsUpdateName = DB::connection('nissin')->update("UPDATE 
                                                                    Influencers                                               
                                                                  SET
                        ...
                        if (strrpos($influencer->img, $urlProfilePic) === false) {
                            ...
                            $resultsUpdateInfluencer = DB::connection('nissin')->update("UPDATE 
                                                                            Influencers 
                                                                          SET
                                                                            img       = '{$urlProfilePic}'
                                                                          WHERE 
                        ...
                        foreach ($responseStories['items'] as $story){
                            ...
                            $resultsInfluencerHasStory = DB::connection('nissin')->select("SELECT
                                                                                * 
                                                                             FROM 
                                                                                Historias 
                                                                             WHERE 
                            ...
                            if(!$resultsInfluencerHasStory){
                                ...
                                if($story['media_type'] == 1){ #imagem
                                    if($this->temHashtagPrograma){
                                        ...
                                        $resultsInsertStory = DB::connection('nissin')->table('Historias')
                                            ->insert(
                                                [
                                                    'aplicativo'            => 1,
